...
namespace Aries\Gateway\Zarinpal;
...
class Zarinpal extends PortAbstract implements PortInterface
{
	...
	{
		switch (Setting::get('zarinpal-type', 'normal')) {
			case 'zarin-gate':
	...
	{
		if (!$this->callbackUrl)
			$this->callbackUrl = Setting::get('zarinpal-cb-url', '/');
	...
	protected function sendPayRequest()
	{
		...
		$fields = array(
			'MerchantID' => Setting::get('zarinpal-merchant'),
			'Amount' => $this->amount,
			'CallbackURL' => $this->getCallback(),
			'Description' => $this->description ? $this->description : Setting::get('zarinpal-description', ''),
			'Email' => $this->email ? $this->email :Setting::get('zarinpal-email', ''),
			'Mobile' => $this->mobileNumber ? $this->mobileNumber : Setting::get('zarinpal-mobile', ''),
		);
	...
	{
		$server = Setting::get('zarinpal-server', 'test');
		switch ($server) {
			case 'iran':
